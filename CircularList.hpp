...
public:
  ...
  T * first ()
  {
    return m_root.next != &m_root ? Accessor::fromListEntry(m_root.next) : NULL;
  }

  T * last ()
  {
    return m_root.prev != &m_root ? Accessor::fromListEntry(m_root.prev) : NULL;
  }

  T * next ( T * item )
  {
    ListEntry * tmp = Accessor::toListEntry( item )->next;
    return tmp != &m_root ? Accessor::fromListEntry(tmp) : NULL;
  }
